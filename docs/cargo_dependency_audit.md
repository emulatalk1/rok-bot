...
> **Status (snapshot — see [Cargo.toml](../Cargo.toml) for the live set):** This audit was the pre-implementation pin set. The shipped runtime stack as of **v0.1.8** is `core-graphics 0.25`, `core-foundation 0.10`, `objc2 0.6`, `objc2-foundation 0.3` (NSString + NSArray + NSError), `objc2-screen-capture-kit 0.3` (SCStream + SCShareableContent + SCScreenshotManager + block2 + objc2-core-graphics + objc2-core-foundation + libc features), `objc2-core-graphics 0.3` (CGImage + CGDataProvider + CGColorSpace + CGGeometry), `objc2-core-foundation 0.3` (CFCGTypes), `block2 0.6`, `dispatch2 0.3`, `libc 0.2`, `thiserror 2`, `anyhow 1`, `tracing 0.1`, `tracing-subscriber 0.3`, `image 0.25` (PNG only), `imageproc 0.26` (rayon feature), `tempfile 3`. Notable rejections from this audit: `xcap` (replaced by SCK), `enigo` (replaced by direct `core-graphics::event::CGEvent::post`), `screencapturekit` 1.5.x (its `build.rs` invokes `xcrun --show-sdk-platform-path` which needs full Xcode — `objc2-screen-capture-kit` is the CLT-friendly alternative). Still out of scope: `tesseract` (OCR — no current need), `reqwest`/`tokio`/`dotenvy`/`rand` (no networking yet).
>
> **v0.1.8 dependency changes:** Dropped `objc2-app-kit` (the v0.1.x NSRunningApplication bundle-ID lookup is now driven off `SCWindow.owningApplication.bundleIdentifier`, sourced from SCK enumeration). Added the SCK stack: `objc2`, `objc2-screen-capture-kit`, `objc2-core-graphics`, `objc2-core-foundation`, `block2`, `dispatch2`, `libc`. All six SCK crates were spike-validated at version 0.3.x in `spikes/p8-spike/Cargo.toml` (commit `9c576a9`); feature sets transferred verbatim to the parent `Cargo.toml`. The `libc` feature on `objc2-screen-capture-kit` unlocks `SCRunningApplication.processID() -> libc::pid_t`; `libc` is also a direct dep for the `O_NOFOLLOW` flag in `capture.rs`'s symlink-safe file open. v0.1.8 hard-requires macOS 14.0+ (SCScreenshotManager availability) — documented in `Cargo.toml` and `docs/setup.md`.
